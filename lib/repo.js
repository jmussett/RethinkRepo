...
var internalQuery = require("./query.js");
...
module.exports = function(dbName, host, port, newLogger) {
    ...
    return {
        ...
        newModel: function (name) {
            if(!isInitialised) {
                throw new RepositoryError("Models can only be created after the Repository has been initialised", logger);
            }
        ...
        query: function(modelName) {
            ...
            if (!modelExists(models, modelName)) {
                throw new RepositoryError("Model '" + modelName + "' has not been registered, please register this model using Repository.Register()");
            }

            var existingModel = _(models).find(function(m) {
                return m.name === modelName;
            });

            // If the selected model has been registered, return a new query object
            var newQuery = internalQuery(r.table(modelName), existingModel, true);
